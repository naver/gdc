...
from gdc.gdc import GDCTrainer, BaselineGDCTrainer
...
def load_neptune(config):
    if os.environ.get("NEPTUNE_PROJECT_NAME", ""):
        ...
        project = neptune.init(os.environ["NEPTUNE_PROJECT_NAME"])
...
def main(config):
    ...
    for epoch in tqdm(range(start_epoch, int(np.ceil(config["steps"]/config['batch_size'])), 1)):
        ...
        for f in all_feature_values:
            ...
            train_logs[f'mu_{f}  (desired = {desired_value} ) '] = current_moment
        ...
        if config.get('eval', True) and (epoch+1) % config.get('eval_interval', 20) == 0:
            ...
            for f in all_feature_values:
                desired_value = config['desired_moments'][f]
                current_moment = all_feature_values[f].mean().item()
                eval_logs[f'Eval/mu_{f}  (desired = {desired_value} ) '] = current_moment
