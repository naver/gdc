...
__all__ = ['GDCTrainer','BaselineGDCTrainer']
...
from .core import logprobs_from_logits, flatten_dict, stack_dicts
...
class GDCTrainer(BaseTrainer):
    ...
    def __init__(self, model_cls, tokenizer, sampling_function, features, **params):
        ...
        # we pass the sampling method to be able to use it for for the calculation of TVD
    ...
    def compute_rewards(self, scores, model_input, gen_len):
        ...
        orig_logprob = orig_logprob[:, -gen_len:] # (might be not neccesary) we only keep prob regardless of the query

        # calculate b(x)
        orig_logprob = orig_logprob.detach() # we don't backprob to original model
        orig_logprob = torch.sum(orig_logprob, dim=-1) # log(a(x)) of shape [batch size]
    ...
    def loss(self, scores, query, response, model_input):
        ...
        # Advantage is equal to Reward since no baseline hereby
        advantage = rewards
    ...
    def compute_optimal_lambdas(self, sample_size=4096, n_iters=1000, lr=.5):
        ...
        # check for zero-occurring features.
...
class BaselineGDCTrainer(GDCTrainer):
    """
    A distributional DPG trainer that subtracts a baseline from the score in its optimisation step. In other words, the loss
